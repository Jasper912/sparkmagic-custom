...
class LivyReliableHttpClient(object):
    ...
    def post_completion(self, session_id, kind, code, cursor):
        data = {
            "code": code,
            "cursor": cursor,
            "kind": kind
        }
        return self._http_client.post(self._completion_url(session_id), [200], data).json()
    ...
    @staticmethod
    def _completion_url(session_id):
        return "/sessions/{}/completion".format(session_id)
