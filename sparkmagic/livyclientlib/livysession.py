...
class LivySession(ObjectWithGuid):
    def __init__(self, http_client, properties, ipython_display,
        ...
        self._session_language = properties.pop("session_language", None)
        self._session_name = properties.pop("session_name", None)

        spark_conf = properties.get(constants.SESSION_CONF_PARAM, {})
        self._spark_yarn_queue = spark_conf.get(constants.SPARK_YARN_QUEUE_PARAM, "")
        ...
        self._extra_session_properties = {}
    ...
    def start(self):
        ...
        try:
            ...
            self._save_extra_session_properties(self.id)
            ...
            # 去掉创建session成功后显示livysession信息
            #html = get_sessions_info_html([self], self.id)
            #self.ipython_display.html(html)

            if self.kind == constants.SESSION_KIND_SQL:
                self.sql_context_variable_name = "sql"
                command = Command("show databases")
                (success, out) = command.execute(self)
                if success:
                    return
                else:
                    raise SqlContextNotFoundException(u"SparkSession is not available.")
    ...
    def refresh_status_and_info(self):
        ...
        # 取session的时候判断session是否属于当前用户
        system_name = getpass.getuser()
        if response[u'proxyUser'] != system_name:
            return None
    ...
    def session_language(self):
        if not self._session_language:
            self._session_language = self._get_session_language(self.id)

        return self._session_language

    def _get_session_language(self, session_id):
        extra_session_properties = self.get_extra_session_properties(session_id)
        return extra_session_properties.get("session_language", None)

    @property
    def session_name(self):
        if not self._session_name:
            self._session_name = self._get_session_name(self.id)

        return self._session_name

    def _get_session_name(self, session_id):
        extra_session_properties = self.get_extra_session_properties(session_id)
        return extra_session_properties.get("session_name", None)

    @property
    def spark_yarn_queue(self):
        if not self._spark_yarn_queue:
            self._spark_yarn_queue = self._get_spark_yarn_queue(self.id)

        return self._spark_yarn_queue

    def _get_spark_yarn_queue(self, session_id):
        extra_session_properties = self.get_extra_session_properties(session_id)
        return extra_session_properties.get(constants.SPARK_YARN_QUEUE_PARAM, None)

    def get_extra_session_properties(self, session_id):
        if not self._extra_session_properties:
            self._extra_session_properties = self._get_extra_session_properties(session_id)

        return self._extra_session_properties

    def _get_extra_session_properties(self, session_id):
        username = getpass.getuser()
        user_livy_dir = os.path.join(constants.LIVY_INFO_PATH, username)
        file_name = f"session_{session_id}.json"
        file_path = os.path.join(user_livy_dir, file_name)
    ...
    def _save_extra_session_properties(self, session_id):
        """
            save extra session info
        """
        # FIXME storage redis or zookeeper
        username = getpass.getuser()
        user_livy_dir = os.path.join(constants.LIVY_INFO_PATH, username)
        os.makedirs(user_livy_dir, exist_ok=True)
        ...
        data = {
            ...
            constants.SPARK_YARN_QUEUE_PARAM: self.spark_yarn_queue
        }
